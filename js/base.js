...
function AppInfo()// 浏览器类型版本判断
{
    ...
    if(userAgent.indexOf("edge") != -1)
        browser.appname = 'edge';
    else if (/(msie|firefox|opera|chrome)\D+(\d[\d.]*)/.test(userAgent))
    {
...
JqueryUnsupportedAnimation.prototype = {
    ...
    //匀速翻转,用法同上
    rotateY:function (dom,angle_start,angle_end,speed,fn){
        var isAnimated = true;
        if(isAnimated)
        {
            isAnimated = false;
            ...
            speed > 10 ? speed : console.log("参数(4)错误:动画时间不能小于10");
            var rotateY_jsq = setInterval(function ()
            {
                angle_start += step;
                if(Math.abs(angle_end - angle_start) < 1){
                    isAnimated = true;
                    angle_start = angle_end;
                    clearInterval(rotateY_jsq);
                    if($.isFunction(fn))
                        fn();
                }
                dom.css({transform:"rotateY(" + angle_start + "deg)"});
            },10);
        }
    }
}
