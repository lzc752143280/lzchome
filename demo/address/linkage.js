...
a = CITYS[16].city[0].area[4];//汉阳
b = CITYS[16].city[0].name;//武汉
c = CITYS[16].name;//湖北
$("body").text(c+"省 "+b+"市 "+a);
*/
...
$(CITYS).each(function(i,e)
{
  $("#province").append("<option value="+i+">"+ CITYS[i].name +"</option>");
});
$("#province").on("change",function(){
    province = $("#province option:selected").val();
    $("#city option:first,#area option:first").nextAll().remove();
    $(CITYS[province].city).each(function(i,e)
    {
        $("#city").append("<option value="+i+">"+ CITYS[province].city[i].name +"</option>");
    });
});
$("#city").on("change",function(){
    city = $("#city option:selected").val();
    $("#area option:first").nextAll().remove();
    $(CITYS[province].city[city].area).each(function(i,e)
    {
        $("#area").append("<option value="+i+">"+ CITYS[province].city[city].area[i] +"</option>");
    });
});
$("#area").on("change",function(){
    ...
    console.log(area);
    if(area>=0)
        $("p").text(CITYS[province].name + CITYS[province].city[city].name+ "市" + CITYS[province].city[city].area[area]);
});
